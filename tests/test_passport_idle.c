...
static void test_sleeps_at_thirty_seconds(void)
{
    ...
    assert(idle.awake == 1);
    assert(passport_idle_on_tick(&idle, 14999U) == PASSPORT_IDLE_NONE);
    assert(idle.awake == 1);
...
int main(void)
{
    passport_idle_t idle;
    passport_idle_init(&idle);
    assert(passport_idle_on_tick(&idle, 15000U) == PASSPORT_IDLE_SLEEP);
    assert(passport_idle_set_unread(&idle, 2) == PASSPORT_IDLE_WAKE);
    assert(passport_idle_on_tick(&idle, 15000U) == PASSPORT_IDLE_SLEEP);
